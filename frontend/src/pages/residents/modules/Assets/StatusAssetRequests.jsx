...
const StatusAssetRequests = () => {
  ...
  useEffect(() => {
    const fetchRequests = async () => {
      try {
        ...
        // If data is an object with other structure, try to extract relevant info
        else if (data && typeof data === 'object') {
          console.log('API Response structure:', data);
          // Fallback to default data if we can't parse the response
  ...
  const getAssetName = (item) => {
    ...
    if (item.asset && typeof item.asset === 'object') {
      return item.asset.name || item.asset.asset_name || item.asset.title || 'Unknown Asset';
    }
  ...
  const getReceiptNumber = (item) => {
    return item.receipt_number || null;
  };
      ...
      <>
        ...
        <main className="ml-64 pt-20 p-8 bg-white min-h-screen font-sans">
          <div className="flex justify-center items-center h-64">
            <div className="text-lg text-gray-600">Loading asset requests...</div>
          </div>
        </main>
      </>
    );
