                ...
                {cart.map((item, idx) => (
                  <li key={idx} className="flex items-center justify-between py-3">
                    <div className="flex items-center gap-4">
                      <img src={item.asset.image && item.asset.image.startsWith('/storage/') ? `http://localhost:8000${item.asset.image}` : item.asset.image} alt={item.asset.name} className="w-12 h-12 rounded-lg object-cover border" />
                      <div>
      ...
      {detailModal.open && detailModal.asset && (
        <Modal onClose={() => setDetailModal({ open: false, asset: null })}>
          <div className="max-w-lg w-full p-6">
            ...
            <img src={detailModal.asset.image && detailModal.asset.image.startsWith('/storage/') ? `http://localhost:8000${detailModal.asset.image}` : detailModal.asset.image} alt={detailModal.asset.name} className="w-full h-56 object-cover rounded-xl mb-4" />
